...
const features = [
  ...
  },
  {
    ...
    body: "One workspace for your business, with per-location attribution for every review and case.",
  },
];

function WalkingClient({
  x,
  delay,
  leaving = false,
}: {
  ...
  return (
    <g className="client-position" transform={`translate(${x} 0)`}>
      <g
        className={`vector-client ${leaving ? "vector-client-leaving" : ""}`}
        style={{ "--walk-delay": `${delay}s` } as React.CSSProperties}
      >
...
function RatingScene({ quiet = false }: { quiet?: boolean }) {
  return (
    <svg
      className={`rating-vector-scene ${quiet ? "rating-vector-quiet" : "rating-vector-busy"}`}
      viewBox="0 0 620 310"
      role="img"
      aria-label={
        quiet
          ? "A customer walking away from a poorly rated business"
          : "Customers walking toward a highly rated business"
      }
    >
      <defs>
        <linearGradient
          id={quiet ? "shop-wall-quiet" : "shop-wall-busy"}
          x1="0"
          y1="0"
          x2="1"
          y2="1"
        >
          <stop className="shop-wall-light" />
          <stop offset="1" className="shop-wall-dark" />
        </linearGradient>
        <linearGradient
          id={quiet ? "shop-roof-quiet" : "shop-roof-busy"}
          x1="0"
          y1="0"
          x2="0"
          y2="1"
        >
          <stop className="shop-roof-light" />
          <stop offset="1" className="shop-roof-dark" />
        </linearGradient>
        <radialGradient id={quiet ? "window-glow-quiet" : "window-glow-busy"}>
          <stop className="window-glow-core" />
          <stop offset="1" className="window-glow-edge" />
        </radialGradient>
        <filter
          id={quiet ? "soft-glow-quiet" : "soft-glow-busy"}
          x="-60%"
          y="-60%"
          width="220%"
          height="220%"
      ...
      {!quiet && (
        <>
          ...
          <g className="money-token money-token-a">
            <circle r="19" />
            <text x="0" y="8">
              $
            </text>
          </g>
          <g className="money-token money-token-b">
            <circle r="22" />
            <text x="0" y="8">
              $
            </text>
          </g>
          <g className="money-token money-token-c">
            <circle r="24" />
            <text x="0" y="9">
              $
            </text>
      ...
      {quiet && (
        <>
          ...
          <g className="negative-review-vector" transform="translate(285 94) rotate(12)">
            <path d="M-30-24 H30 Q38-24 38-16 V19 Q38 27 30 27 H-6 L-20 42 V27 H-30 Q-38 27-38 19 V-16 Q-38-24-30-24Z" />
            <path className="negative-star" d="M0-15 5-5 17-3 8 5 10 17 0 11-10 17-8 5-17-3-5-5Z" />
            <path className="review-speed" d="M-55-10h-19M-54 2h-28M-51 14h-16" />
          </g>
        </>
      )}
      ...
      <g className="vector-shop" transform={quiet ? "translate(405 112)" : "translate(365 91)"}>
        <ellipse
          ...
          rx="136"
          ry="74"
          filter={`url(#${quiet ? "soft-glow-quiet" : "soft-glow-busy"})`}
        />
        <path
          className="shop-side"
          d={quiet ? "M175 51 205 64V178H175Z" : "M194 49 224 64V188H194Z"}
        />
        <rect
          className="shop-main"
          x="0"
          y="47"
          width={quiet ? "176" : "196"}
          height={quiet ? "131" : "141"}
          fill={`url(#${quiet ? "shop-wall-quiet" : "shop-wall-busy"})`}
        />
        <rect
          className="shop-sign"
          x={quiet ? "73" : "82"}
          ...
          height="37"
          rx="3"
          fill={`url(#${quiet ? "shop-roof-quiet" : "shop-roof-busy"})`}
        />
        <path
          className="shop-roof-vector"
          d={
            quiet
              ? "M-14 43Q-14 35-6 35H181L205 48V60H-14Z"
              : "M-15 43Q-15 34-7 34H202L225 48V61H-15Z"
          }
          fill={`url(#${quiet ? "shop-roof-quiet" : "shop-roof-busy"})`}
        />
        {!quiet && (
          <g className="shop-awning-vector">
            <path d="M-4 61H199L190 100H4Z" />
            <path d="M20 61 15 100M50 61 48 100M80 61 81 100M111 61 114 100M143 61 147 100M174 61 181 100" />
          </g>
        )}
        <rect
          className="shop-window-vector"
          x="19"
          y={quiet ? "78" : "111"}
          width={quiet ? "72" : "91"}
          height={quiet ? "63" : "62"}
          rx="2"
          fill={`url(#${quiet ? "window-glow-quiet" : "window-glow-busy"})`}
        />
        <path className="window-bars" d={quiet ? "M55 78v63M19 109h72" : "M65 111v62M19 142h91"} />
        <rect
          className="shop-door-vector"
          x={quiet ? "116" : "132"}
          y={quiet ? "72" : "99"}
          width="48"
          height={quiet ? "106" : "89"}
      ...
      {!quiet ? (
        <g className="client-stream">
          {[52, 126, 200, 274, 348, 422].map((x, index) => (
            <WalkingClient key={x} x={x} delay={index * -0.82} />
          ))}
        </g>
      ) : (
        <g className="leaving-stream" transform="translate(252 0) scale(-1 1)">
          <WalkingClient x={0} delay={0} leaving />
        </g>
      )}
    </svg>
  );
...
export function ReferenceLanding({
  ...
  function openScanner() {
    setScannerOpen(true);
    window.setTimeout(
      () => document.querySelector<HTMLInputElement>("#reference-review-url")?.focus(),
      50,
    );
  }
      ...
      <div className="reference-wrap">
        <section className="reference-hero">
          <div className="reference-copy">
            ...
            <p>
              Checked against Google&apos;s review content policy.
              <br />
              No policy violation, no report. It&apos;s that simple.
            </p>
            <div className="reference-actions">
              <button
                type="button"
                onClick={openScanner}
                className="reference-gradient-button reference-primary-action"
              >
              ...
              <a href="#how" className="reference-outline-button reference-secondary-action">
          ...
          <div className="reference-proof" aria-label="Illustrative review analysis animation">
            ...
            <div className="proof-business">
              <span className="proof-business-mark">
                <ShieldCheck />
              </span>
              <div>
            ...
            <div className="proof-review-stack">
              <article className="proof-row proof-row-one">
                <div>
                  <span className="reference-avatar">ML</span>
                  <b>Maria L.</b>
                  <time>1 week ago</time>
                </div>
                <span className="proof-row-stars">★★★★★</span>
                <p>Best dental experience I&apos;ve ever had. Dr. Chen is incredible.</p>
              </article>
              <article className="proof-row proof-row-bad proof-row-two">
                <div>
                  <span className="reference-avatar">JR</span>
                  <b>James R.</b>
                  <time>3 weeks ago</time>
                </div>
                <span className="proof-row-stars">★☆☆☆☆</span>
                <p>
                  Go to SmileDeals instead — call 555-0134 and mention this review for 50% off...
                </p>
              ...
              <article className="proof-row proof-row-three">
                <div>
                  <span className="reference-avatar">TW</span>
                  <b>Tom W.</b>
              ...
              <article className="proof-row proof-row-bad proof-row-four">
                <div>
                  <span className="reference-avatar">MK</span>
                  <b>Mike T.</b>
                  <time>2 months ago</time>
                </div>
                <span className="proof-row-stars">★☆☆☆☆</span>
                <p>
                  The highway traffic near here was a nightmare today. Terrible city planning...
                </p>
                <strong className="proof-issue-badge">Policy issue detected</strong>
                <span className="proof-scan-line" />
              </article>
        ...
        <section id="platforms" className="reference-platforms">
          <div className="reference-platforms-copy">
            <h2>
              Finally, a way for businesses to protect themselves and <span>fight back.</span>
            </h2>
            ...
            <p>
              Google review scanning is live today — every other platform joins the same simple
              paste → AI check → report → track flow as it rolls out.
            </p>
          </div>
                    ...
                    <span className="platform-glyph" style={{ color: platform.color }}>
                      {Icon ? (
                        <Icon
                          className="size-6"
                          fill={
                            platform.icon === Star || platform.icon === Youtube
                              ? platform.color
                              : "none"
                          }
                        />
          ...
          <section id="scan" className="reference-scan-panel animate-rise">
            <div>
              <span className="reference-kicker">
                <Sparkles className="size-3.5" /> Live Google review scan
              </span>
              <h2>Paste a review URL</h2>
              <p>
                We fetch the real business and available review text before AI checks the policy
                evidence.
              </p>
            </div>
            <form onSubmit={onScan}>
              <input
                id="reference-review-url"
                value={url}
              ...
              <Button type="submit" disabled={busy} className="reference-gradient-button">
                Scan review <ArrowRight className="size-4" />
              </Button>
            </form>
          </section>
        ) : null}
        ...
        <section className="reference-promise">
          <div className="reference-promise-left">
            ...
            <p>
              The AI tells you honestly when a review breaks the rules — and when it doesn&apos;t.
              No fake flags, no wasted reports.
            </p>
          </div>
          <div className="reference-promise-right">
            <div className="reference-promise-item">
              <h3>
                <Scale className="promise-icon" /> Real Policy Checks. The Legit Way.
              </h3>
              <p>
                No spammed appeals, no fake claims. Every report cites the actual platform policy,
                the review evidence and a confidence score — and Google makes the final call.
              </p>
            </div>
            <div className="reference-promise-item">
              <h3>
                <BadgeCheck className="promise-icon" /> Only Real Outcomes
              </h3>
              <p>
                Every case is tracked from report to Google&apos;s decision. If a review stays up,
                your dashboard says so — that&apos;s the deal.
              </p>
            </div>
          </div>
        </section>
        ...
        <section id="how" className="reference-section reference-three-steps">
          ...
          <div className="three-step-grid">
            <article className="three-step-card three-step-links">
              ...
              <p>
                Just send us the review links. That&apos;s it. We don&apos;t need access to your
                Google Business Profile. Zero risk to your account, zero setup.
              </p>
            </article>
            ...
            <article className="three-step-card three-step-case">
              <div className="step-visual step-envelope" aria-hidden="true">
                ...
                <span className="case-note case-note-one">
                  <i />
                  <i />
                  <i />
                </span>
                <span className="case-note case-note-two">
                  <i />
                  <i />
                  <i />
                </span>
                <span className="case-note case-note-three">
                  <i />
                  <i />
                  <i />
                </span>
              ...
              <p>
                No spammed appeals and no unsupported claims. Our AI checks the review against
                published policy, organises the evidence and prepares the strongest legitimate
                report.
              </p>
            </article>
            ...
            <article className="three-step-card three-step-outcome">
              ...
              <p>
                Submit the prepared report to Google, then track every case in your dashboard.
                Google makes the final decision, and the real result stays visible.
              </p>
            </article>
          </div>
        </section>
        ...
        <section className="review-impression" aria-label="Google rating impact comparison">
          <div className="rating-showdown">
            ...
            <article className="rating-business rating-you">
              ...
              <div className="rating-score rating-score-low">
                <strong>3.9</strong>
                <span aria-label="3 out of 5 highlighted stars">
                  <i>★</i>
                  <i>★</i>
                  <i>★</i>
                  <i>★</i>
          ...
          <div className="reference-features">
            {features.map((feature, index) => (
              <article key={feature.title}>
                <img src={feature.icon} alt="" style={{ animationDelay: `${index * 310}ms` }} />
                <h3>{feature.title}</h3>
                <p>{feature.body}</p>
              </article>
            ))}
        ...
        <section className="audit-finale" aria-labelledby="audit-finale-title">
          <div className="audit-finale-copy">
            ...
            <div className="audit-finale-actions">
              ...
              <a href="#how" className="audit-call-action">
                How It Works <ArrowRight />
              </a>
            </div>
            <p className="audit-finale-note">
              100% transparent. Real policy evidence, no guaranteed removals, no fake outcomes.
            </p>
          </div>

          <div className="audit-shield-stage" aria-hidden="true">
            <i className="audit-ray audit-ray-one" />
            <i className="audit-ray audit-ray-two" />
            <i className="audit-ray audit-ray-three" />
          ...
          <div className="audit-chat-panel">
            ...
            <p>
              Paste the link to the review that is hurting you. Our AI reads the real review and
              checks the evidence.
            </p>
            <button type="button" onClick={openScanner} className="audit-chat-action">
              Open scanner <ArrowRight />
            </button>
        ...
        <footer className="reference-footer">
          <p>
            © {new Date().getFullYear()} Removal Work. Reputation intelligence for multi-location
            brands.
          </p>
          <p>
            Removal Work is not affiliated with Google. Removal outcomes are determined solely by
            Google after reviewing each report.
          </p>
        </footer>
      </div>
      ...
      {chatOpen ? (
        <div className="reference-chat">
          <div className="reference-chat-bubble">
            ...
            <button type="button" onClick={() => setChatOpen(false)} aria-label="Dismiss">
          ...
          <button
            type="button"
            onClick={openScanner}
            className="reference-chat-avatar"
            aria-label="Start a review audit"
          >
            RW
          </button>
        </div>
